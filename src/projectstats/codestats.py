...
class GitCodeStats:
    ...
    :ivar cloc_path: Path to the cloc command for running cloc stats
    ...
    """
    def __init__(self, git_paths, output_dir, cloc_path):
        """
    ...
    def compute_code_stats(self, load_cached_results=True, cache_results=True):
        """
        ...
        if load_cached_results and os.path.exists(self.cache_file_cloc) and os.path.exists(self.cache_file_commits):
            print("Loading cached results: %s" % self.cache_file_cloc)
            with open(self.cache_file_cloc) as f:
                self.cloc_stats = yaml.load(f, Loader=yaml.FullLoader)
            print("Loading cached results: %s" % self.cache_file_commits)
            with open(self.cache_file_commits) as f:
                self.commit_stats = yaml.load(f, Loader=yaml.FullLoader)
    ...
    def compute_summary_stats(self, date_range):
        ...
        self.summary_stats = {'sizes': repo_sizes_aligned_df,
